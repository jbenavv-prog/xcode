...
export default function Productos() {
  ...
  useEffect(() => {
    async function obtenerDatos() {
      const peticion = await fetch("/productosList/productos.json");
  ...
  return (
    <Layout>
      ...
      >
        ...
        <div className="ml-3 text-sm font-normal">Hola a todos!!!!!!</div>
        <button
          type="button"
      ...
      <div className="flex flex-wrap justify-between">
        {productosEstado ? (
          productosEstado.map((elemento, index) => {
            return (
              <Producto
                key={index}
                producto={elemento}
...
export function Producto({ producto, alerta }) {
  return (
    <div className="mb-5 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
      <a href="#">
